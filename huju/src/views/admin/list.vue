<script setup>
    ...
    const tablePage = reactive({
        ...
        try{
            const response=await axios.get(
                'http://182.92.158.95:8020/api/user/list',
                {headers:{ 
    ...
    async function handleAddSubmit() {
        try{
            const response =await axios.get(
                'http://182.92.158.95:8020/api/user/export',
                    {
    ...
    <el-container width="100vw" height="100vh">
      ...
      <el-container>
        <el-header style="background-color: white;">
            ...
            <el-breadcrumb-item :to="{ path: 'landing' }">线索数据</el-breadcrumb-item>
            </el-breadcrumb>
            ...
            <div class="top">
                <!-- 新增落地页 -->
                    <div style="margin:20px 0;">
                        <el-button type="primary" @click="openAddDialog">导出数据</el-button>
                    </div>
                <!-- 新增落地页 -->
            ...
            <el-button type="primary" @click="handleSearch">搜索</el-button>
                ...
                <!-- 表格 -->
                 <el-table :data="fileTabledate" border style=" width: 100%; height: 100%">
                    ...
                    <el-table-column prop="name" label="页面来源" width="180">
                        <template #default="scope">
                        {{ scope.row.path }}
                        </template>
                    </el-table-column>
                    <el-table-column prop="create_time" label="渠道" width="160">
                        <template #default="scope">
                        {{ new Date(scope.row.create_time*1000).toLocaleString () }}
                        </template>
                    </el-table-column>
                    <el-table-column prop="enable" label="备注" width="180">
                        <template #default="scope">
                        {{ scope.row.enable===1?'已启用':'已停用' }}
                        </template>
                    </el-table-column>
                    <el-table-column prop="enable" label="登录时间" width="200">
                        <template #default="scope">
                        {{ scope.row.enable}}
                        </template>
                    </el-table-column>
